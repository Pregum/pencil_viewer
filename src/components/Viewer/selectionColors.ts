/**
 * Selection Colors の集計と一括置換のロジック。
 *
 * コンポーネントから分けてあるのは、純粋関数だから単体でテストしたいのと、
 * コンポーネント以外を component ファイルから export すると Fast Refresh が
 * 効かなくなるため（react-refresh/only-export-components, #78）。
 */
...
export function replaceColor(
  ...
  oldColor: string,
  newColor: string,
): { nodes: PenNode[]; replaced: number } {
  let replaced = 0;
  const upper = oldColor.toUpperCase();
  const walkVal = (v: unknown): unknown => {
    if (typeof v === 'string' && v.toUpperCase() === upper) {
      replaced++;
      return newColor;
    }
