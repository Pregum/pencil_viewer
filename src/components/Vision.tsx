...
const enContent: VisionContent = {
  ...
  roadmap: {
    ...
    phases: [
      ...
      { phase: 'AI Review', status: 'done', statusLabel: 'Done', desc: 'Cloudflare Workers AI design analysis' },
      { phase: 'AI Design Generator', status: 'done', statusLabel: 'Done', desc: 'Prompt → .pen frame via Cloudflare Workers AI (Cmd+K)' },
      { phase: 'Design Doc Export', status: 'done', statusLabel: 'Done', desc: 'Markdown design docs (Notion importable)' },
      { phase: 'Figma-like Editing', status: 'done', statusLabel: 'Done', desc: 'Shape tools, pen tool, snap guides, align/distribute, etc.' },
      { phase: 'Components & Variants', status: 'done', statusLabel: 'Done', desc: 'Reusable components with variants via live ref resolution' },
      { phase: 'Auto Layout & Constraints', status: 'done', statusLabel: 'Done', desc: 'Flex layout + responsive constraints on parent resize' },
      { phase: 'Styles Library', status: 'done', statusLabel: 'Done', desc: 'Named Color / Text / Effect styles' },
      { phase: 'Boolean Ops & Flatten', status: 'done', statusLabel: 'Done', desc: 'Union / Subtract / Intersect / Exclude + outline stroke' },
      { phase: 'Masking', status: 'done', statusLabel: 'Done', desc: 'Use-as-mask (Cmd+Alt+M) via SVG alpha mask' },
      { phase: 'Prototype & Smart Animate', status: 'done', statusLabel: 'Done', desc: 'onTap frame navigation with interpolated transitions' },
      { phase: 'Dev Mode Inspect', status: 'done', statusLabel: 'Done', desc: 'CSS / Tailwind / SwiftUI / Compose snippets' },
      { phase: 'Comment Mode', status: 'done', statusLabel: 'Done', desc: 'In-canvas pins with threads, resolve/reopen' },
      { phase: 'Figma Import', status: 'next', statusLabel: 'Next', desc: 'Figma API \u2192 .pen conversion' },
      { phase: 'Notion API', status: 'next', statusLabel: 'Next', desc: 'Direct Notion page creation/update' },
...
const jaContent: VisionContent = {
  ...
  roadmap: {
    title: 'ロードマップ',
    phases: [
      { phase: 'Viewer', status: 'done', statusLabel: '完了', desc: 'コア .pen レンダリング' },
      { phase: 'Editor', status: 'done', statusLabel: '完了', desc: 'ノード編集、Vim モード' },
      { phase: 'Collab', status: 'done', statusLabel: '完了', desc: 'P2P WebRTC 編集' },
      { phase: 'AI 連携', status: 'done', statusLabel: '完了', desc: 'MCP ブリッジ、REST API' },
      { phase: 'Quality Gate', status: 'done', statusLabel: '完了', desc: '5 つの UI 状態監査' },
      { phase: 'AI Review', status: 'done', statusLabel: '完了', desc: 'Cloudflare Workers AI デザイン分析' },
      { phase: 'AI Design Generator', status: 'done', statusLabel: '完了', desc: 'プロンプトから .pen Frame を生成 (Cmd+K)' },
      { phase: '設計書エクスポート', status: 'done', statusLabel: '完了', desc: 'Markdown 設計書（Notion インポート可）' },
      { phase: 'Figma 風編集', status: 'done', statusLabel: '完了', desc: 'シェイプツール、ペン、スナップガイド、整列 等' },
      { phase: 'Components & Variants', status: 'done', statusLabel: '完了', desc: '再利用コンポーネント、バリアント + live ref 解決' },
      { phase: 'Auto Layout / Constraints', status: 'done', statusLabel: '完了', desc: 'Flex レイアウト + レスポンシブ制約' },
      { phase: 'Styles Library', status: 'done', statusLabel: '完了', desc: '名前付き Color / Text / Effect スタイル' },
      { phase: 'Boolean / Flatten', status: 'done', statusLabel: '完了', desc: 'Union / Subtract / Intersect / Exclude + Outline stroke' },
      { phase: 'Masking', status: 'done', statusLabel: '完了', desc: 'Use as mask (Cmd+Alt+M)、SVG alpha mask' },
      { phase: 'Prototype & Smart Animate', status: 'done', statusLabel: '完了', desc: 'onTap でフレーム遷移、補間トランジション' },
      { phase: 'Dev Mode Inspect', status: 'done', statusLabel: '完了', desc: 'CSS / Tailwind / SwiftUI / Compose スニペット' },
      { phase: 'Comment Mode', status: 'done', statusLabel: '完了', desc: 'キャンバス上のピン + スレッド、解決/再オープン' },
      { phase: 'Figma Import', status: 'next', statusLabel: '次回', desc: 'Figma API → .pen 変換' },
      { phase: 'Notion API', status: 'next', statusLabel: '次回', desc: 'Notion ページ直接作成/更新' },
      { phase: 'CI Integration', status: 'next', statusLabel: '次回', desc: 'GitHub Actions デザインレビュー' },
    ],
  },
};
