...
export function PenToolCreator({ svgRef, svgScale }: Props) {
  ...
  // ツールを抜けたら state をリセット。effect ではなくレンダー中に調整する
  // （React 公式の "Adjusting state when a prop changes"）。effect でやると
  // 破棄したはずの下書きが 1 フレーム残る (#78)。
  ...
  let ghostD = '';
  // ドラッグ中かどうかは dragHandle（pointerdown で立て、pointerup で null に戻す）
  // から分かる。レンダー中に ref を読まない (react-hooks/refs, #78)。
  if (hover && inprog.length >= 1 && dragHandle === null) {
    const last = inprog[inprog.length - 1];
  ...
  return (
    <g pointerEvents="none">
      ...
      {/* 確定済み path */}
      {confirmedD && <path d={confirmedD} stroke="#4F46E5" strokeWidth={sw} fill="none" />}
      {/* アンカーとハンドル */}
      {inprog.map((a, i) => (
        <g key={i}>
          {a.inHandle && (
